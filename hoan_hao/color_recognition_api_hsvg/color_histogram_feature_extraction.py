...
from color_recognition_api_hsvg import knn_classifier as knn_classifier
def color_histogram_of_test_image(test_src_image):
    ...
    chans1  = cv2.split(image_rgb)
    chans2  = cv2.split(image_hsv)
    chans = (chans2[0],chans2[1],chans2[2],chans1[1])
    # print(chans)
    colors = ('h', 's','v','g')
    features = []
    feature_data = ''
    ...
    for (chan, color) in zip(chans, colors):
        ...
        if counter == 1:
            ...
            green = str(elem)
            feature_data= hue + ',' + saturation + ',' + value + ',' + green
            # print(feature_data)
    with open('test_hsvg.data', 'w') as myfile:
...
def color_histogram_of_training_image(img_name):
    ...
    with open('training_hsvg.data', 'a') as myfile:
