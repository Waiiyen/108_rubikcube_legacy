...
def control_GPIO():
    ...
    if GPIO.input(button_4) == GPIO.LOW:
        ...
        for i in range (1,random.randint(15,20)):
            ...
            randoms = random.choice(functions)
            print(randoms)
            if randoms == "run_U":
                sign = "U"
                run_U(num) 
            ...
            # random_function(num)
            # print(sign.upper())
            step = "{}{}".format(sign.upper(),num)
            print(step)
            puzzle_step = "{} {}".format(puzzle_step,step)
        print(puzzle_step)
        GPIO.output(DIR__, 0)
        GPIO.output(ENA__, 0)
        ...
        time.sleep(10)
        print("solve_step")
        # print(len(puzzle_step))
        for t in range(len(puzzle_step)-2,0,-3):
            # print("hello")
            # print(puzzle_step[t])
            # print(puzzle_step[t+1])
            # print(puzzle_step[t-1])
            
            if puzzle_step[t] ==  "U":
                sign = "U"
                run_U(4-int(puzzle_step[t+1]))
            if puzzle_step[t] ==  "R":
                sign = "R"
                run_R(4-int(puzzle_step[t+1]))
            if puzzle_step[t] == "F":
                sign = "F"
                run_F(4-int(puzzle_step[t+1]))
            if puzzle_step[t] == "D":
                sign = "D"
                run_D(4-int(puzzle_step[t+1]))
            if puzzle_step[t] == "L":
                sign = "L"
                run_L(4-int(puzzle_step[t+1]))
            if puzzle_step[t] == "B":
                sign = "B"
                run_B(4-int(puzzle_step[t+1]))
            # print(sign)
            step = "{}{}".format(sign.upper(),4-int(puzzle_step[t+1]))  
            # print(step)
            solve_step = "{} {}".format(solve_step,step)
        print(solve_step)
        GPIO.output(DIR__, 0)
        GPIO.output(ENA__, 0)
        for t in range(10100):
